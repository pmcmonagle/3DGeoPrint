...
parser.add_argument("s", help="scale", metavar="S")
...
with open("database/registry.json", "r") as jsonfile:
...
def readrow(f, x, width):
    ...
    while len(row) < modelWidth:
        row.append((struct.unpack("i", f.read(INTLENGTH))[0] * relativeHeight) * targetS + 1)
        f.seek(f.tell() + INTLENGTH * (targetR-1))
    minValue = min(minValue, min(row))
    return row

def readFile(name, x, y, width, height):
    with open("database/data/{0}".format(name), "r") as f:
        result = list()
